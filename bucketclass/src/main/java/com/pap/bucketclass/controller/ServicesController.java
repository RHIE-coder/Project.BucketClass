...
public class ServicesController {
	...
	public @ResponseBody ResultItems<Services> providerPageableServiceActive(
		...
		String defaultSort ="serviceModifiedDate";
		Pageable pageable = PageRequest.of(page - 1, size, Sort.by(defaultSort).descending());
		Page<Services> serviceList = listingService.listingActivePage(pageable);
		return new ResultItems<Services>(serviceList.stream().collect(Collectors.toList()), page, size, serviceList.getTotalElements());
	}
	...
	//		return resModel;
	//	}
	public @ResponseBody ResponseModel createTemplate(@ModelAttribute CreateTemplateModel model, Principal principal) {
		ResponseModel resModel = new ResponseModel();
		if (templateService.createTemplate(model, principal) != null) {
			resModel.setRes("success");}else{resModel.setRes("fail");
			}
		return resModel;
	}
